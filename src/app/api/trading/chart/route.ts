 ...
 * - period: 조회 기간 (1d, 1w, 1m, 3m, 1y) - 기본값: 1m
...
export async function GET(request: Request) {
  try {
    ...
    if (assetType === 'coin') {
      chartData = await marketService.getCoinChart(symbol, interval, period);
      source = marketService.isUpbitAvailable() ? 'UPBIT' : 'MOCK';
    } else {
      chartData = await marketService.getStockChart(symbol, interval, period);
