...
class MarketService {
   ...
   */
  async getStockChart(symbol: string, interval: string = '1d', period: string = '1m'): Promise<UnifiedChartData[]> {
    const cacheKey = `stock:chart:${symbol}:${interval}:${period}`;
    ...
    return marketCache.getOrSet(cacheKey, async () => {
      ...
      return this.generateMockChartData(symbol, interval, period);
    }, 60000);
  }
   ...
   */
  async getCoinChart(symbol: string, interval: string = '1d', period: string = '1m'): Promise<UnifiedChartData[]> {
    const cacheKey = `coin:chart:${symbol}:${interval}:${period}`;
   ...
   */
  private generateMockChartData(symbol: string, interval: string, period: string = '1m'): UnifiedChartData[] {
    ...
    let count: number;
    if (interval === '1m') {
      count = Math.min(days * 24 * 60, 500); // minutes in period, max 500
    } else if (interval === '5m') {
      count = Math.min(days * 24 * 12, 500);
    } else if (interval === '15m') {
      count = Math.min(days * 24 * 4, 500);
    } else if (interval === '1h') {
      count = Math.min(days * 24, 500);
    } else if (interval === '1d') {
      count = Math.min(days, 365); // days in period, max 365
    } else if (interval === '1w') {
      count = Math.min(Math.ceil(days / 7), 52); // weeks in period, max 52
    } else {
      count = days;
    }
